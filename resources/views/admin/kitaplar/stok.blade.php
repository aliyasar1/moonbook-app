...
@section('content')
    <div class="row mx-lg-5 flex-fill">
        <div class="col-xl-12 col-lg-12 col-md-9">
            <table class="table" style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis">
                ...
                @foreach($stoklar as $stok)
                    <tr class="text-center h-100">
                        ...
                        <td class="py-4 px-6 text-sm font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            <img
                                class="ratio3x2" style="width: 50px"
                        ...
                        <td class="align-middle">{{ \Illuminate\Support\Str::limit($stok->kitap->aciklama, 30) }}</td>
                        <td class="align-middle">{{ '₺ '. $stok->kitap->fiyat }}</td>
                        <form method="post" action="{{ route('satici.kitaplar.stok_put', ['stok' => $stok->id]) }}"
                              enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <td class="align-middle"><input type="text" class="form-control w-25 mx-auto"
                                                            id="stok_adeti"
